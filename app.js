...
) && !window.location.search.includes('real-firebase=true');

if (useMock) {
  console.log("Mission Jeet: Local environment or missing Firebase detected. Injecting LocalStorage-based Firebase Mock.");
  ...
  const mockStorage = {
    ...
    getCollectionData(path) {
      ...
      const raw = mjStorage.getItem(key);
      return raw ? JSON.parse(raw) : {};
    },
    saveCollectionData(path, data) {
      const key = this.getCollectionKey(path);
      mjStorage.setItem(key, JSON.stringify(data));
      this.notify(path);
    },
  ...
  const mockAuth = {
    ...
    onAuthStateChanged(callback) {
      ...
      const savedUser = mjStorage.getItem('mj_mock_auth_user');
      if (savedUser) {
    ...
    signInWithPopup(provider) {
      return new Promise((resolve) => {
        ...
        mockAuth.currentUser = user;
        mjStorage.setItem('mj_mock_auth_user', JSON.stringify(user));
    ...
    signOut() {
      return new Promise((resolve) => {
        mockAuth.currentUser = null;
        mjStorage.removeItem('mj_mock_auth_user');
...
function initApp() {
  ...
  mjStorage.setItem('mj_gemini_key', BUNDLED_KEY);
...
function applyProfile() {
  ...
  const photo = mjStorage.getItem('mjPhoto_'+currentUser.uid) || currentUser.photoURL || '';
...
async function uploadProfilePhoto(input) {
  ...
  reader.onload = e => {
    ...
    mjStorage.setItem('mjPhoto_'+currentUser.uid, url);
...
function openMarksModal(id) {
  ...
  document.getElementById('marksTestInfo').textContent = label + ' - ' + (t.type || 'Test') + ' - ' + (t.date || '');
...
async function generateMentorInsights(forceRefresh = false) {
  ...
  const apiKey = mjStorage.getItem('mj_gemini_key') || '';
