...
function updateActivePrediction(pred, targetPeriod) {
    ...
    const isGated = pred.confidence < CONFIG.MIN_CONFIDENCE;

    if (isGated) {
        valueEl.textContent = "HOLD";
        valueEl.className = "pred-value hold";
        valueEl.style.color = "var(--accent-gold)";
        valueEl.style.textShadow = "0 0 8px var(--accent-gold)";
        
        if (confEl) confEl.textContent = pred.confidence + '% (BLOCKED)';
        if (stratEl) stratEl.textContent = "LOW CONFIDENCE SIGNAL";
...
function updateStats() {
    ...
    // Live Accuracy Gauge - SVG progress circle
...
function updateConfidenceTrend() {
    const chart = document.getElementById('confidenceTrendChart');
    if (!chart) return;
    const history = HistoryManager.load().slice(0, 30).reverse();
    if (history.length === 0) {
        chart.innerHTML = `<span style="color: var(--text-secondary); font-size: 11px;">Establishing prediction context...</span>`;
        return;
    }
    
    let html = '';
    history.forEach(h => {
        if (!h.prediction_confidence) return;
        const conf = h.prediction_confidence;
        const color = conf >= 75 ? 'var(--accent-green)' : conf >= 60 ? 'var(--accent-gold)' : 'var(--accent-blue)';
        const height = Math.max(8, ((conf - 40) / 60) * 100); // Normalize 40%-100% to fill the graph height
        html += `
            <div class="trend-bar-wrap" style="flex: 1; display: flex; flex-direction: column; align-items: center; height: 100%; justify-content: flex-end; position: relative;" title="Period: ${h.issue_number} | Conf: ${conf}%">
                <div style="width: 70%; height: ${height}%; background: ${color}; opacity: 0.85; border-top-left-radius: 2px; border-top-right-radius: 2px; box-shadow: 0 0 8px ${color}; transition: height 0.5s ease;"></div>
                <span style="font-size: 7px; color: var(--text-secondary); margin-top: 4px; font-family: 'Share Tech Mono', monospace; transform: scale(0.85);">${h.issue_number.slice(-3)}</span>
            </div>`;
    });
...
function updateAnalytics() {
    ...
    if (stratList) {
        // Model Performance Ranking - sort descending by recentAccuracy
        const stats = engine.getStrategyStats().sort((a, b) => b.recentAccuracy - a.recentAccuracy);
...
function updateModels(prediction) {
    ...
    if (featureList) {
        const features = [
            { name: 'Markov Chain O3', key: 'markov_chain' },
            { name: 'Bayesian Updating', key: 'bayesian_update' },
            { name: 'Monte Carlo Sim', key: 'monte_carlo' },
            { name: 'Hidden Markov Model', key: 'hidden_markov' },
            { name: 'Frequency Distribution', key: 'frequency_dist' },
            { name: 'Double Exp Smoothing', key: 'rolling_trend' },
            { name: 'Shannon Entropy', key: 'entropy_anal' },
            { name: 'Chi-Square Prob', key: 'chi_square' },
            { name: 'Autocorrelation', key: 'autocorr' },
            { name: 'Pattern Persistence', key: 'pattern_persist' },
            { name: 'RSI Momentum', key: 'momentum_det' },
            { name: 'Z-Score Reversion', key: 'mean_revert' },
            { name: 'Streak Analysis', key: 'streak_anal' },
            { name: 'Volatility Measure', key: 'volatility_anal' }
        ];
